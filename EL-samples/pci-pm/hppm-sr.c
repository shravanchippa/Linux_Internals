/*
 * A kernel module to demonstrate the power management - suspend & resume
...
#define VENDOR_ID               0x10EC 	/* realtek 8139 */
...
int rtl8139_suspend(struct pci_dev *pdev, int state)
{
    printk("PCI suspend\n");
    return 0;
}

int rtl8139_resume(struct pci_dev *pdev)
{
    printk("PCI resume\n");
    return 0;
}
